...
if [ "$RESET" = "1" ] || [ "$SOUNDMODEM_RUNNING" = "0" ]; then
    ...
    echo ""
    echo "===================================================================="
    echo "SET THESE 4 THINGS, then click OK (only needed after a reset):"
    echo "   1. Protocol:      GEOSCAN 2.7 9600bd"
    echo "   2. Frequency:     436.270 MHz"
    echo "   3. Input device:  Monitor of Alferov_Pipeline"
    echo "   4. KISS Server:   port $KISS_PORT, box CHECKED"
    echo ""
    echo "TIP: tick 'Minimized window on startup' in the same dialog so it"
    echo "stops popping up in front every time you reset."
    echo "===================================================================="
    read -p "Press Enter once all 4 are set and you clicked OK... "
else
